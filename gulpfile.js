...
const $cwd              = ( $full_path ) => $path.relative( process.cwd(), $full_path );
const isUndefined       = val => val === undefined;
const vs_config_value   = function( $array, $src, $dest ) {
...
const $notice           = ( $notice ) => $gulp.src( '' ).pipe( $notify( $notice ) );
...
const vs_compile_scss     = function( $_path ) {
	let $instance = new VS_Gulp( $config.scss, $_path, vs_config_value( $config.scss, $_path ) );
	$instance.sourcemap();
...
const vs_compile_js       = function( $_path ) {
	let $instance = new VS_Gulp( $config.js, $_path, vs_config_value( $config.js, $_path ) );
	$instance.combine_files().webpack();
	$instance.save();
	return $instance;
};
const vs_compile_all_scss = function( $current_loop ) {
	const $scss_files_keys = Object.keys( $config.scss );
	if( false === isUndefined( $scss_files_keys[ $current_loop ] ) ) {
		vs_compile_scss( $scss_files_keys[ $current_loop ] );
		vs_compile_all_scss( $current_loop + 1 );
	}
};
const vs_compile_all_js   = function( $current_loop ) {
	const $js_files_keys = Object.keys( $config.js );
	if( false === isUndefined( $js_files_keys[ $current_loop ] ) ) {
		vs_compile_js( $js_files_keys[ $current_loop ] );
		vs_compile_all_js( $current_loop + 1 );
	}
};

const VS_Gulp                   = function( $vars, $src, $dist ) {
	this.src      = $src;
	this.dist     = $dist;
	this.notices  = '';
	this.vars     = $vars;
...
$gulp.task( 'watch:scss', function( callback ) {
	...
	for( $src in $config.scss ) {
		$gulp.watch( $src, ( a, b ) => vs_compile_scss( $cwd( a[ 'path' ] ) ) );
		if( true !== isUndefined( $config.scss[ $src ][ 'watch' ] ) ) {
			$gulp.watch( $config.scss[ $src ][ 'watch' ], () => vs_compile_scss( $src ) );
		}
	}
	callback();
} );
$gulp.task( 'watch:js', function( callback ) {
	...
	for( $src in $config.js ) {
		$gulp.watch( $src, ( a, b ) => vs_compile_js( $cwd( a[ 'path' ] ) ) );
		if( true !== isUndefined( $config.js[ $src ][ 'watch' ] ) ) {
			$gulp.watch( $config.js[ $src ][ 'watch' ], () => vs_compile_js( $src ) );
		}
	}
	callback();
} );
